...
public class DBProject extends ProjectInstance{
	...
	EventBus eb;
	public DBProject(MongoClient client, String projectInstanceDB, EventBus eb) {
		super();
		this.client = client;
	...
	public Future<JsonObject> save() {
		...
		client.save(projectInstanceDB, toJson()).onSuccess(result ->
		{
			saveSuccess.complete(toJson());
		}).onFailure(fail ->{
