...
public class ShuntingYard {
    ...
    public static Token[] convertToRPN(final String expression, final Map<String, Function> userFunctions,
        ...
        final Stack<Token> functionStack = new Stack<Token>();
        final HashMap<Token,Integer> parenthesisCounter = new HashMap<Token, Integer>();        
        final Tokenizer tokenizer = new Tokenizer(expression, userFunctions, userOperators, variableNames, implicitMultiplication);        
        Token currentfunction = null;
        while (tokenizer.hasNext()) {
            ...
            case Token.TOKEN_FUNCTION:
                stack.add(token);
                functionStack.add(currentfunction);
                currentfunction = token;
                parenthesisCounter.put(currentfunction,0);
                break;
            ...
            case Token.TOKEN_PARENTHESES_OPEN:
                ...
                {
                	parenthesisCounter.put(currentfunction, parenthesisCounter.get(currentfunction)+1);
                	if(parenthesisCounter.get(currentfunction) == 1)
                	{
                		output.add(token);
                	}
                }                
            ...
            case Token.TOKEN_PARENTHESES_CLOSE:
                ...
                stack.pop();
                if(currentfunction != null && parenthesisCounter.get(currentfunction) == 0)
                {
                	output.add(token);
                	currentfunction = functionStack.isEmpty() ? null : functionStack.pop();                	
