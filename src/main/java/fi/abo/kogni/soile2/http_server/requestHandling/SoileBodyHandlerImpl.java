 ...
 */
public class SoileBodyHandlerImpl implements BodyHandler {

	private static final Logger LOG = LoggerFactory.getLogger(SoileBodyHandlerImpl.class);
	...
	private String gitFileLakeRoot;
		
	public SoileBodyHandlerImpl() {
		this(true, DEFAULT_UPLOADS_DIRECTORY);
	}

	public SoileBodyHandlerImpl(boolean handleFileUploads) {
		this(handleFileUploads, DEFAULT_UPLOADS_DIRECTORY);
	}

	public SoileBodyHandlerImpl(String uploadDirectory) {
		this(true, uploadDirectory);
	}

	private SoileBodyHandlerImpl(boolean handleFileUploads, String uploadDirectory) {
	...
	public void handle(RoutingContext ctx) {
		if(ctx.normalizedPath().matches("/task/.*/resource/.*") && ctx.request().method() == HttpMethod.POST)
		{
			...
			String targetFolder = gitFileLakeRoot +  File.separator + taskid;			
			handleOrig(ctx, targetFolder);
		}
		else
		{
			handleOrig(ctx, baseUploadDir);
		}

	}


	public void handleOrig(RoutingContext context, String uploadsDir) {
		...
		if (!((RoutingContextInternal) context).seenHandler(RoutingContextInternal.BODY_HANDLER)) {
			...
			final BHandler handler = new BHandler(context, isPreallocateBodyBuffer ? parsedContentLength : -1, uploadsDir);
			request
	...
	public BodyHandler setUploadsDirectory(String uploadsDirectory) {
		this.baseUploadDir = uploadsDirectory;
		return this;
	}
	...
	private class BHandler implements Handler<Buffer> {
		...
		public BHandler(RoutingContext context, long contentLength, String targetFolder) {
			...
			if (isMultipart || isUrlEncoded) {
				...
				if (handleFileUploads) {
					makeUploadDir(context.vertx().fileSystem(), uploadDir);
				}
				context.request().uploadHandler(upload -> {
					...
					if (handleFileUploads) {
						...
						String uploadedFileName = createUploadFile().getPath();
		...
		private void makeUploadDir(FileSystem fileSystem, String uploadDir) {
			if (!fileSystem.existsBlocking(uploadDir)) {
				fileSystem.mkdirsBlocking(uploadDir);
			}
		}
		...
		{
			File f = new File(uploadDir, UUID.randomUUID().toString());
			while(!f.exists())
			{
				f = new File(uploadDir, UUID.randomUUID().toString());	
			}
			return f;
		}
	}
