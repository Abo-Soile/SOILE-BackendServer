...
let expName = 'minitest';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'images/apple.jpg', 'path': 'images/apple.jpg'},
    {'name': 'images/pear.jpg', 'path': 'images/pear.jpg'}
  ]
});
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    frameRemains = 0.4 + 1.5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (image.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      image.setAutoDraw(false);
    }
    
    // *text* updates
    if (t >= 0.4 && text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
